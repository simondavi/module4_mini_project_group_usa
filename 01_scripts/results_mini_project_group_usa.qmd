...
# Output the results
cat("Average time per item for each respondent (in seconds):", average_time_per_item, "seconds\n")
...
# Missing data flag: TRUE if the entire SCIPOP battery (all 8 original items) is missing
# Respondents missing the focal construct entirely cannot contribute to substantive analyses
data_raw <- data_raw %>%
  mutate(
    missing_flag = rowSums(!is.na(select(., SCIPOP_common, SCIPOP_good,
                                             SCIPOP_advantage, SCIPOP_cahoots,
                                             SCIPOP_influence, SCIPOP_involved,
                                             SCIPOP_lifeexp, SCIPOP_rely))) == 0
  )
...
miss_model_trust_sci <- glm(
  miss_trust_sci ~ DEM_AGE + DEM_EDU + Duration + DEM_GENDER,
  data = dplyr::bind_cols(data_raw, domain_missing_usa),
  family = binomial()
)

summary(miss_model_trust_sci)

model_df <- dplyr::bind_cols(data_raw, domain_missing_usa) |>
  dplyr::filter(
    !is.na(DEM_AGE),
    !is.na(DEM_EDU),
    !is.na(Duration),
...
+ Missing Data: entire SCIPOP battery missing (all 8 original items NA)

...
# Write flag back to data_raw as logical TRUE / FALSE / NA
# rd_scipop$id = row number in data_raw (scipop was selected without reordering)
data_raw <- data_raw |>
  mutate(row_id = row_number()) |>
  left_join(
    rd_scipop_flagged |>
      transmute(row_id = id,
                rd_flag = as.logical(flag_rd)),
    by = "row_id"
  ) |>
  select(-row_id)
```
...
# Write flag back to data_raw as logical TRUE / FALSE / NA
# all5pt$ID_num = row_number() from data_raw, so id maps to data_raw row numbers.
# Respondents absent from all5pt_RP_clean (dropped via drop_na) receive NA.
data_raw <- data_raw |>
  mutate(row_id = row_number()) |>
  left_join(
    all5pt_RP_clean |>
      transmute(row_id = id,
                rs_flag = as.logical(flag_suspect)),
    by = "row_id"
  ) |>
  select(-row_id)
```
...
n_total      <- nrow(final_flags)
n_excluded   <- sum(final_flags$final_exclude, na.rm = TRUE)
pct_excluded <- round(mean(final_flags$final_exclude, na.rm = TRUE) * 100, 1)
mean_flags   <- round(mean(final_flags$n_flags, na.rm = TRUE), 2)

```

Applying the two-flag exclusion rule, **`r n_excluded` of `r n_total` US respondents (`r pct_excluded`%) are marked for removal** from the analytical sample. The mean flag count of `r mean_flags` indicates that most respondents triggered at most one quality indicator, confirming that the individual flags capture partly distinct concerns rather than the same respondents repeatedly. Two concurrent flags used together strikes a reasonable balance between data quality and data loss, in retaining the large majority of the sample while targeting the subset who exhibit statistically problematic response behaviour.
